...
#### 📋 类别 1：项目技术文档（保留在 `docs/`）
**定义**: 面向用户和开发者的纯技术文档，不涉及敏感信息
**示例**:
- ✅ `01-quickstart.md` - 快速入门
- ✅ `02-login-system.md` - 登录系统说明
- ✅ `03-database-design.md` - 数据库设计（使用占位符）
- ✅ `05-api-reference.md` - API 参考
- ✅ `README.md` - 文档导航
- ✅ `10-changelog.md` - 更新日志（公开版本）

**规则**:
- 只包含技术说明和架构设计
- 使用占位符代替真实密钥（如 `your-project`、`your-api-key`）
- 面向开源社区，可以公开发布

#### 🔒 类别 2：内部敏感文档（移动到 `小白AI信息/`）
**定义**: 包含敏感信息、内部配置或决策记录的文档
**示例**:
- 🔴 API 密钥配置和安全策略
- 🔴 第三方服务配置（阿里云、Apple Developer）
- 🔴 内部使用指南和两台电脑配置
- 🔴 版本开发记录和安全事故记录
- 🔴 RLS 策略决策和技术方案讨论
...
**分类检查清单**:
- [ ] 文档是否包含真实 API Keys、密码、URL？
  - 是 → 移动到 `小白AI信息/docs-security/`
- [ ] 文档是否包含第三方服务配置（阿里云、Apple）？
  - 是 → 移动到 `小白AI信息/docs-third-party/`
- [ ] 文档是否是内部使用指南或配置说明？
  - 是 → 移动到 `小白AI信息/docs-internal/`
- [ ] 文档是否是版本开发记录或事故记录？
  - 是 → 移动到 `小白AI信息/docs-versions/` 或 `docs-security/`
- [ ] 文档是否只包含技术说明和占位符？
  - 是 → 保留在 `docs/`

**位置说明**:
- **项目技术文档**: `/Users/shawn/Downloads/小白AI/docs/`
- **内部敏感文档**: `/Users/shawn/Downloads/同步空间/Claude code/小白AI信息/`
