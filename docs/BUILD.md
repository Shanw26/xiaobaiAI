...
**重要**: 如果发现 Git 历史中包含敏感信息，必须彻底清理后才能推送。

#### ❌ 常见错误做法

1. **只修改文件，不清理 Git 历史**
   - 问题：Git 历史中仍然包含旧版本的敏感信息
   - 后果：即使文件已修改，`git log` 仍能看到历史密钥

2. **在文档中记录真实密钥（即使是作为示例）**
   - 问题：记录安全修复的文档本身又包含敏感信息
   - 示例：`MEMORY_SECURITY_OPTIMIZATION.md` 包含真实密钥
   - 后果：文档被提交后，所有人都能看到真实密钥

3. **代码中的 fallback 值使用真实密钥**
   - 问题：环境变量不可用时的备用方案泄露了真实密钥
   - 示例：`electron/database.js:538` 的 fallback URL
   - 后果：打包后的应用可能包含真实密钥
...
# 4. 创建新的初始提交
git commit -m "Initial commit: v2.20.2 (完全安全版)"
...
```markdown
# 发现的问题
- 真实 Supabase URL: `https://example-project.supabase.co` ← 真实密钥！
- 真实 API Key: `sb_publishable_abc123...` ← 真实密钥！
```

✅ **正确示例**：
```markdown
# 发现的问题
- 真实 Supabase URL: `https://your-project.supabase.co` (占位符)
- 真实 API Key: `sb_publishable_YOUR_KEY_HERE` (占位符)
- 影响文件: docs/01-quickstart.md, electron/database.js 等
- 影响: 15+ 个文件
```

**3. 代码中的 fallback 值也必须使用占位符**

❌ **错误示例**：
```javascript
const supabaseUrl = process.env.SUPABASE_URL ||
                    'https://example-project.supabase.co'; // ← 真实密钥！
```
...
| 次数 | 发现问题 | 清理内容 | Git 提交数 |
|------|---------|---------|-----------|
| **第1次** | Git 历史中有敏感信息 | 删除 Git 历史 | 2 个 |
| **第2次** | 文档中包含真实密钥 | 替换文档、删除临时文件 | 1 个 |
| **第3次** | 代码 fallback + 记录文档 | 清除所有硬编码、删除记录文档 | **1 个** ✅ |
...
```bash
# 1. 检查 Git 历史中是否还有真实密钥
git log --all -S "sb_publishable_" --oneline
git log --all -S "your-project-id" --oneline

# 2. 检查当前文件中是否有硬编码
git ls-files | xargs grep -l "sb_publishable_[a-zA-Z0-9_-]\{30,\}"
git ls-files | xargs grep -l "your-project-id"
...
# 4. 验证敏感文件是否已排除
grep "^\.env$" .gitignore
grep "^key.md$" .gitignore
grep "^MEMORY.md$" .gitignore  # 如果包含真实信息
```
...
5. **最终验证至关重要**
   - 使用 `git log -S "关键词"` 搜索历史
   - 使用 `git ls-files | xargs grep` 搜索当前文件
   - 确保 100% 清除后再推送
...
export const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY || 'sb_publishable_YOUR_ANON_KEY_HERE';
```
