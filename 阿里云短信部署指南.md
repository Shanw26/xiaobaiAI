...
**当前项目的 AccessKey：**
- `AccessKey ID`: `LTAI5tHaRfJVTDjSjGetM8Pi`
...
1. 打开 Supabase Dashboard: https://supabase.com/dashboard/project/cnszooaxwxatezodbbxq
2. 进入 `Settings` → `Edge Functions`
3. 在 `Environment Variables` 部分添加以下变量：

```bash
ALIYUN_ACCESS_KEY_ID=LTAI5tHaRfJVTDjSjGetM8Pi
...
Deployed Function send-sms
  URL: https://cnszooaxwxatezodbbxq.supabase.co/functions/v1/send-sms
```
...
```bash
curl -X POST 'https://cnszooaxwxatezodbbxq.supabase.co/functions/v1/send-sms' \
