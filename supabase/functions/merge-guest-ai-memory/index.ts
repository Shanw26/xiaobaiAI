...
interface MergeGuestAiMemoryRequest {
  user_id: string
  device_id: string
}

serve(async (req) => {
  const FUNCTION_NAME = 'merge-guest-ai-memory'
  ...
  try {
    ...
    const { user_id, device_id }: MergeGuestAiMemoryRequest = await req.json()
    ...
    if (userData) {
      ...
      console.log('🗑️  登录用户已有AI记忆，删除游客数据')
